...
def make_action(screen, info, step, env, prev_action):
    ...
    actions_list = {"Right": 1,"Jump": 2, "Left": 6 }

    simple_object_action_mappings = {
    'enemy': 'Jump',                   # Jump to avoid or defeat Goomba.                    # Jump on Koopa to defeat or use its shell.
    'spiny': 'Right',                   # Can't defeat by jumping, so move right to avoid.
    'piranha_plant': 'Right',           # Move right to pass by when it's retracted.
    'coin': 'Right',                    # Move towards the coin to collect.
    'mushroom': 'Right',                # Move towards the power-up.
    'fire_flower': 'Right',             # Move towards the power-up.
    'star': 'Right',                    # Move towards the power-up.
    'pipe': 'Jump',                     # Jump over the pipe.
    'pit': 'Jump',                      # Jump over the pit.
    'flagpole': 'Right',                # Move towards to finish the level.
    'lakitu': 'Right',                  # Try to move past Lakitu.
    'hammer_bro': 'Right',              # Move to avoid the hammers.
    'buzzy_beetle': 'Jump',             # Jump on to defeat or use its shell.
    'cheep_cheep': 'Right',             # Move right to avoid, especially if it's in the air.
    'bullet_bill': 'Jump',              # Jump to avoid the Bullet Bill.
    'bowser': 'Right',                  # Move to avoid or find an alternative way to defeat.
    'lava': 'Jump',                     # Jump to avoid falling into lava.
    'platform': 'Jump', 
    ...
    # calculate distance 
    action = actions_list.get("Right")
    if mario_locations:
        location_m, dimensions_m, object_name_m = mario_locations[0]
        mario_x, mario_y = location_m 
        
        distance_enemy= 100000
        distance_pipe = 100000
        pipe_exists = False
        pipe_height = 0
        
        object_distances = {
            "enemy": distance_enemy, 
            "pipe": distance_pipe
        }
        
        if len(enemy_locations) != 0:
            location_e, dimensions_e, object_name_e = enemy_locations[0]
            enemy_x, enemy_y = location_e
            distance_enemy  = enemy_x - mario_x
            if (distance_enemy < -40 ):
                distance_enemy = 1000
            
            object_distances["enemy"] = distance_enemy
            
        if len(block_locations) != 0: 
            for block in block_locations:
                location, dimensions, object_name = block
                block_x, block_y = location
                width, height = dimensions
                if (object_name == "pipe"):
                    print("found a pipe")
                    distance_pipe = block_x - mario_x
                    pipe_height = height
                    object_distances["pipe"] = distance_pipe
                    pipe_exists = True
                    break

        if (len(enemy_locations) != 0 or pipe_exists):
            object_distances = {k: v for k, v in sorted(object_distances.items(), key=lambda item: item[1])}
            closest_obj = next(iter(object_distances))
            distance = object_distances.get(closest_obj)
            print("closest_obj : " + closest_obj)
            print("enemy distance : ")
            print(distance_enemy)
            print("pipe distance : ")
            print(distance_pipe)
            print(step)
            
                    
            action = actions_list.get(simple_object_action_mappings.get(closest_obj))

            if (distance <= 40 and prev_action != action):
        ...
        # # I have no strategy at the moment, so I'll choose a random action.
            ...
            while(action == actions_list.get("Jump")):
                action = env.action_space.sample()
            print("new action")
            print(action)
